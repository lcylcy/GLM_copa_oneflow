...
class GLMForMultiTokenCloze(flow.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask, target_ids=None, logit_mask=None, prompt_pos=None):
        ...
        # input_ids = read("/dataset/lichunyou/GLM/input/input_ids").reshape(4,2,256)
        # position_ids = read("/dataset/lichunyou/GLM/input/position_ids").reshape(4,2,2,256)
        # attention_mask = read("/dataset/lichunyou/GLM/input/attention_mask").reshape(4,2)
        # target_ids = read("/dataset/lichunyou/GLM/input/target_ids").reshape(4,2,256)
        # logit_mask = read("/dataset/lichunyou/GLM/input/logit_mask").reshape(4,2,256)
        # prompt_pos = None
