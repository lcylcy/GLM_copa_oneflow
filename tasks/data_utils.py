...
def my_default_collate(batch):
    ...
    for key in batch[0]:
        if type(batch[0][key]) is not np.ndarray:
            new_batch[key] = flow.Tensor(np.random.rand(length))
        else:
            new_batch[key] = flow.Tensor(np.random.rand(length,*batch[0][key].shape))
    
    for i in range(len(batch)):
        for key in batch[i]:
            if type(batch[i][key]) is not np.ndarray:
                new_batch[key][i] = batch[i][key]
            else:
                new_batch[key][i] = flow.Tensor(batch[i][key])
    
    for key in new_batch:
        new_batch[key] = new_batch[key].to(flow.int64)
...
def my_collate(batch):
    ...
    new_batch = my_default_collate(new_batch)
